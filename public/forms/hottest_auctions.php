<?php
  ...
  $query = "SELECT
      ...
      p.pictureName
    FROM items i
    LEFT JOIN ( SELECT itemID, count(itemID) AS numBids FROM bids GROUP BY itemID)
    h ON i.itemID = h.itemID
    JOIN ( SELECT itemID, MAX(bidValue) bidValue FROM bids GROUP BY itemID )
